var app = new Vue({
    ...
    data: {
        ...
        //模板审核通过条件列表
        searchEntity:{templateStatus: '1'},
        //审核状态 0 1 2
        status: ['未审核', '已审核', '审核未通过'],
        ids: [],//用于存储选中的id的值的数组
        //规格未审核条件列表
        searchEntityAudit: {templateStatus: '0'}
    },
    methods: {
        ...
        searchListAudit: function (curPage) {
            axios.post('/typeTemplate/search.shtml?pageNo=' + curPage, this.searchEntityAudit).then(function (response) {
                //获取数据
                app.list = response.data.list;
                //当前页
                app.pageNo = curPage;
...
/*
        ...
        jsonToString:function (strList,key) {
            ...
            var jsonObjectArray = JSON.parse(strList);
            console.log(jsonObjectArray);

            for(var i=0;i<jsonObjectArray.length;i++){
                var obj = jsonObjectArray[i]; //{"id":27,"text":"网络"}
                str+=obj[key]+","
            }
        ...
        updateStatus: function (status) {
            axios.post('/typeTemplate/updateStatus.shtml?status=' + status, this.ids).then(
                function (response) {
                    if (response.data.success) {
                        app.ids = [];
    ...
    //钩子函数 初始化了事件和
    created: function () {
        if (window.location.href.indexOf("template_audit.html") != -1) {
            this.searchListAudit(1)
            alert("调用了searchListAudit模板")
        } else {
            this.searchList(1);
            alert("调用了searchList模板")
        }
