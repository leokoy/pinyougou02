var app = new Vue({
    el: "#app",
    data: {
        pages: 15,
        pageNo: 1,
        list: [],
        entity: {entityParentId:0},
        entity_1: {},
        entity_2: {},
        grade: 1,//默认就是第一级
        parentId: '',
        //审核状态 0 1 2
        status: ['未审核', '已审核', '审核未通过'],
        ids: [],//用于存储选中的id的值的数组
        //模板列表
        templateList: [],
        //当前分类ID
        entityParentId:0,
        //模板测试
        searchEntity:{status:'0'}

    },

    methods: {
        searchList: function (curPage) {
            axios.post('/itemCat/search.shtml?pageNo=' + curPage, this.searchEntity).then(function (response) {
                //获取数据
                app.list = response.data.list;
                //当前页
                app.pageNo = curPage;
                //总页数
                app.pages = response.data.pages;
            });
        },
        //查询所有品牌列表
        findAll: function () {
            console.log(app);
            axios.get('/itemCat/findAll.shtml').then(function (response) {
                console.log(response);
                //注意：this 在axios中就不再是 vue实例了。
                app.list = response.data;

            }).catch(function (error) {

            })
        },
        findPage: function () {
            var that = this;
            axios.get('/itemCat/findPage.shtml', {
                params: {
                    pageNo: this.pageNo
                }
            }).then(function (response) {
                console.log(app);
                //注意：this 在axios中就不再是 vue实例了。
                app.list = response.data.list;
                app.pageNo = curPage;
                //总页数
                app.pages = response.data.pages;
            }).catch(function (error) {

            })
        },
        //该方法只要不在生命周期的
        add: function () {
            this.entity.entityParentId = this.entityParentId;
            axios.post('/itemCat/add.shtml', this.entity).then(function (response) {
                console.log(response);
                if (response.data.success) {
                    app.findByParentId(0);
                    alert("申请成功")
                    app.findAll();
                }
            }).catch(function (error) {
                console.log("1231312131321");
            });
        },
        update: function () {
            axios.post('/itemCat/update.shtml', this.entity).then(function (response) {
                console.log(response);
                if (response.data.success) {
                    app.searchList(1);
                }
            }).catch(function (error) {
                console.log("1231312131321");
            });
        },
        save: function () {
            if (this.entity.id != null) {
                this.update();
            } else {
                this.add();
            }
        },
        findOne: function (id) {
            axios.get('/itemCat/findOne/' + id + '.shtml').then(function (response) {
                app.entity = response.data;
            }).catch(function (error) {
                console.log("1231312131321");
            });
        },
        dele: function () {
            axios.post('/itemCat/delete.shtml', this.ids).then(function (response) {
                console.log(response);
                if (response.data.success) {
                    app.searchList(1);
                }
            }).catch(function (error) {
                console.log("1231312131321");
            });
        },

        findByParentId: function (parentId) {
            axios.get('/itemCat/findByParentId/' + parentId + '.shtml').then(function (response) {
                app.list = response.data;//lIST
            }).catch(function (error) {
                console.log("1231312131321");
            });
        },
        selectList: function (p_entity) {
            //1.如果当前的等级是 1
            if (this.grade == 1) {
                this.entity_1 = {};
                this.entity_2 = {};
                this.entity.entityParentId = p_entity;
            }

            //2. 如果当前的等级是 2
            if (this.grade == 2) {
                this.entity_1 = p_entity;
                this.entity_2 = {};
                this.entityParentId = p_entity;
                this.entity.entityParentId = p_entity;
            }
            //3.如果当前的等级是 3
            if (this.grade == 3) {
                this.entity_2 = p_entity;
                this.entityParentId = p_entity;
                this.entity.entityParentId = p_entity;
            }
            this.findByParentId(p_entity.id);
        },
        /**
         * 苏红霖 2019，6，14
         * 描述：更新分类审核状态
         * @param status
         */
        updateStatus: function (status) {
            axios.post('/itemCat/updateStatus.shtml?status=' + status, this.ids).then(
                function (response) {
                    if (response.data.success) {
                        app.ids = [];
                        app.searchList(1);
                    }
                }
            )
        },
        /**
         * 苏红霖 2019.6.14
         * 描述：查询所有模板 展示新建列表
         */
        findAllTemplate: function () {
            this.entity = {};
            axios.get('/typeTemplate/findAll.shtml').then(function (response) {
                app.templateList = response.data
            })
        }


    },
    //钩子函数 初始化了事件和
    created: function () {
            this.findByParentId(0);
    }

})
